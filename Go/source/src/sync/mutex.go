...
// for any n < m.
// A successfull call to [Mutex.TryLock] is equivalent to a call to Lock.
...
const (
	...
	//
	// Normal mode ahs considerably better performance as a goroutine can acquire
...
func (m *Mutex) Unlock() {
	...
	if new != 0 {
		...
		// To hide unlockSlow during tracing we skip one extra frame when tracig GoUnblock.
		m.unlockSlow(new)
	}
}
